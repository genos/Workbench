...
bgEnc n r xs = (map xor (zip xs bbsnr), (r^(2^(k+1) `mod` n)) `mod` n)
    where
        bbsnr = bbs n r
        k = length xs
...
bgDec p q ys s = map xor (zip ys bbsnr)
    where
        n = p * q
        k = length ys
        [a,b] = tail (xgcd p q)
