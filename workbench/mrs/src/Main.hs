...
cataM :: (Monad m, Traversable (Base t), RS.Foldable t) => (Base t a -> m a) -> t -> m a
cataM alg = c
  where
    c = alg <=< traverse c . project

anaM :: (Monad m, Traversable (Base t), RS.Unfoldable t) => (a -> m (Base t a)) -> a -> m t
anaM coalg = a
  where
    a = (return . embed) <=< traverse a <=< coalg

paraM :: (Monad m, Traversable (Base t), RS.Foldable t)
...
apoM :: (Monad m, Traversable (Base t), RS.Unfoldable t)
