...
import Prelude
       hiding (foldr, length, lookup, mapM, replicate, sequence)
...
import Text.ParserCombinators.Parsec
       hiding ((<|>), count, many, space)
import Text.PrettyPrint.Leijen
       ((<+>), Doc, Pretty, pretty, space, text)
...
pprAlg (IfNeg t x y) =
  PP.parens $ text "ifNeg" <+> t <+> text "then" <+> x <+> text "else" <+> y
...
algCoProd
  :: (Functor f, Functor g)
  => Algebra f a -> Algebra g a -> Either (f a) (g a) -> a
algCoProd = (|||)

class Functor f =>
      FixPoint f t  | t -> f where
...
memoCata
  :: (Eq (f (Fix f)), Traversable f, Hashable (Fix f))
  => Algebra f a -> Fix f -> a
...
algZygo
  :: Functor f
  => Algebra f b -> (f (a, b) -> a) -> f (a, b) -> (a, b)
algZygo f g = g &&& f . fmap snd

zygo
  :: Functor f
  => Algebra f b -> (f (a, b) -> a) -> Fix f -> a
...
instance Hashable r =>
         Hashable (ExprF r) where
  hashWithSalt s (Const c) = 1 `hashWithSalt` s `hashWithSalt` c
  hashWithSalt s (Var id) = 2 `hashWithSalt` s `hashWithSalt` id
