;;;; utilities.lisp
;;;;
;;;; from chapter 4 of Paul Graham's "On Lisp"
  ...
  (let ((acc nil))
    (dolist (x lst)
      (let ((val (funcall fn x)))
        (if val (push val acc))))
    (nreverse acc)))
; or just (loop for x in lst with y = (funcall fn x) if y collect y) 
...
; Changed from "On Lisp," due to order issues
